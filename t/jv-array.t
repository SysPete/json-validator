...
validate_ok [ 24, 'Sussex', 'Drive' ], $tuple,
  E( '/2', 'Not in enum list: Street, Avenue, Boulevard.' );
...
my $jv = JSON::Validator->new( coerce => { numbers => 1 } );
my @numbers;
...
validate_ok [ 1, 'foo', 1.2 ],
  { type => 'array', items => [ JSON->true, JSON->true, JSON->false ] },
  E( '/2', 'Should not match.' );

validate_ok [ 1, 'foo', 1.2 ], { type => 'array', items => JSON->false },
  E( '/0', 'Should not match.' ),
  E( '/1', 'Should not match.' ), E( '/2', 'Should not match.' );

validate_ok [ 1, 'foo', 1.2 ],
  {
    definitions => { my_true_ref => JSON->true }, type => 'array',
    items       => { '$ref'      => '#/definitions/my_true_ref' }
  };

validate_ok [ 1, 'foo', 1.2 ],
  {
    definitions => { my_false_ref => JSON->false }, type => 'array',
...
validate_ok [ 1, 2 ], { contains => { not => {} } },
  E( '/0', 'Should not match.' ), E( '/1', 'Should not match.' );

validate_ok [ 1, 2 ], { contains => JSON->false },
  E( '/0', 'Should not match.' ), E( '/1', 'Should not match.' );
