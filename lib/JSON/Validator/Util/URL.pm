package JSON::Validator::Util::URL;
use strict;
use warnings;
use overload bool => sub {1}, '""' => sub { shift->to_string }, fallback => 1;

use Encode qw(decode encode);
use JSON::Validator::Util qw(url_escape url_unescape);
use JSON::Validator::Util::Parameters;
use JSON::Validator::Util::Path;
...
sub base {
    ...
    if ( @_ ) {
        ...
        unless ( defined $self->{base} ) {
            $self->{base} = JSON::Validator::Util::URL->new;
        }
        return $self->{base};
    }
}
...
sub ihost {
  ...
  # Decode
  return $self->host(join '.', map { /^xn--(.+)$/ ? decode_punycode $1 : $_ } split(/\./, shift, -1)) if @_;
  ...
  # Encode
  return join '.', map { /[^\x00-\x7f]/ ? ('xn--' . encode_punycode $_) : $_ } split(/\./, $host, -1);
}
...
sub path {
  ...
  # Old path
  $self->{path} ||= JSON::Validator::Util::Path->new;
...
sub query {
  ...
  # Old parameters
  my $q = $self->{query} ||= JSON::Validator::Util::Parameters->new;
  return $q unless @_;
